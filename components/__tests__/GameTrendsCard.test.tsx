...
describe('GameTrendsCard', () => {
  it('renders Strength and Opportunity as separate adaptive message cards', () => {
    const { container } = render(<GameTrendsCard trends={dto()} mode="18" loading={false} error={null} onRetry={jest.fn()} onLogRound={jest.fn()} />);
  ...
  it('shows Early Signal and free-safe copy without SG terminology', () => {
    const { container } = render(<GameTrendsCard trends={dto(5)} mode="18" loading={false} error={null} onRetry={jest.fn()} onLogRound={jest.fn()} />);
  ...
  it('renders a mode-empty CTA and retry state', () => {
    const onLogRound = jest.fn();
    const view = render(<GameTrendsCard trends={projectGameTrendsForViewer(resolveGameTrendsMode({ rounds: [], mode: '9' }), 'free')} mode="9" loading={false} error={null} onRetry={jest.fn()} onLogRound={onLogRound} />);
    expect(screen.getByText('No 9-hole rounds yet')).toBeInTheDocument();
    fireEvent.click(screen.getByRole('button', { name: 'Add Round' }));
    expect(onLogRound).toHaveBeenCalled();
    view.rerender(<GameTrendsCard trends={null} mode="9" loading={false} error="Game Trends could not load." onRetry={jest.fn()} onLogRound={onLogRound} />);
  ...
  it('uses grammatically correct mode-specific empty-state copy', () => {
    ...
    render(<GameTrendsCard trends={empty} mode="18" loading={false} error={null} onRetry={jest.fn()} onLogRound={jest.fn()} />);
  ...
  it('renders one-sided, Balanced and Building profile states without forcing missing roles', () => {
    ...
    const view = render(<GameTrendsCard trends={strengthOnly} mode="18" loading={false} error={null} onRetry={jest.fn()} onLogRound={jest.fn()} />);
    expect(screen.getByText('Strength')).toBeInTheDocument();
    ...
    view.rerender(<GameTrendsCard trends={opportunityOnly} mode="18" loading={false} error={null} onRetry={jest.fn()} onLogRound={jest.fn()} />);
    expect(screen.queryByText('Strength')).not.toBeInTheDocument();
    expect(screen.getByText('Opportunity')).toBeInTheDocument();
    expect(view.container.querySelectorAll('.game-trends-message')).toHaveLength(3);

    view.rerender(<GameTrendsCard trends={balanced} mode="18" loading={false} error={null} onRetry={jest.fn()} onLogRound={jest.fn()} />);
    expect(screen.getByText('Balanced Game')).toBeInTheDocument();
    expect(view.container.querySelectorAll('.game-trends-message')).toHaveLength(3);

    view.rerender(<GameTrendsCard trends={dto(2)} mode="18" loading={false} error={null} onRetry={jest.fn()} onLogRound={jest.fn()} />);
  ...
  it('shows one aggregate confidence badge with explanatory copy', () => {
    render(<GameTrendsCard trends={dto()} mode="18" loading={false} error={null} onRetry={jest.fn()} onLogRound={jest.fn()} />);
